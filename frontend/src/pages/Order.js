      ...
      {data.length === 0 ? (
        ...
        data.map((item, index) => (
          <div key={item._id + index} className="mb-6">
            <h2 className="text-md md:text-xl font-semibold text-slate-800 mb-2">
                  ...
                  {item?.productDetails.map((product, idx) => (
                    ...
                    >
                      <img
                        src={product.image[0]}
                        alt={product.name}
                        className="max-w-24 max-h-24 min-w-24 min-h-24 object-contain shadow-lg p-2 rounded"
                      />
                      <div className="flex flex-col justify-center">
                        <h3 className="font-medium line-clamp-2 md:line-clamp-1 text-md md:text-lg">{product.name}</h3>
